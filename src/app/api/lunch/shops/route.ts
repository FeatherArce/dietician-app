...
export async function POST(request: NextRequest) {
  try {
    ...
    const response: PostShopResponse = {
      success: true,
      message: "商店已新增",
      data: {
        shop,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // delete shop menu first due to foreign key constraint
    const deletedMenus = await menuService.deleteMenusByShopId(data.id);
    console.log("Deleted menus associated with shop:", deletedMenus);
    ...
    return NextResponse.json(
      { message: "Failed to delete shop", success: false, error },
      { status: 500 }
    );
  }
}
