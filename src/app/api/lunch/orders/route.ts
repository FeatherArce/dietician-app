...
export async function POST(request: NextRequest) {
  try {
    ...
    // 檢查是否已有訂單存在
    const existingOrder = await orderService.getUserOrderForEvent(
      session.user?.id,
      event_id
    );
    if (existingOrder) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      return NextResponse.json(
        { error: "缺少訂單 ID 陣列", success: false },
        { status: 400 }
      );
    }

    const deletedOrders = await orderService.deleteOrders(ids);
    const response = {
      data: { orderIds: ids, orders: deletedOrders },
      message: "訂單已刪除",
      success: true,
    };
