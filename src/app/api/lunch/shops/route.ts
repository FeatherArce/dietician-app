...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ shops, success: true });
        
    } catch (error) {
        console.error('GET /api/lunch/shops error:', error);
        return NextResponse.json(
            { error: 'Failed to fetch shops', success: false }, 
            { status: 500 }
        );
    }
}
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!data.name) {
            return NextResponse.json(
                { error: '缺少必要欄位: name', success: false }, 
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { error: 'Failed to create shop', success: false }, 
            { status: 500 }
        );
    }
}
