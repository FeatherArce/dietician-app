...
export async function PUT(
  ...
  try {
    ...
    if (data.order_deadline) {
      const rawOrderDeadline = data.order_deadline;
      const newOrderDeadline = new Date(rawOrderDeadline);
      data.order_deadline = newOrderDeadline;
    }
    if (data.start_at) {
...
export async function DELETE(
  ...
  try {
    ...
    const resolvedParams = await params;
    console.error(
      `DELETE /api/lunch/events/${resolvedParams.id} error:`,
      error
    );
    return NextResponse.json(
